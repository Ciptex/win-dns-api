...
function doDnsSet(req, res, next) {
	...
	if( RRType != "A" && RRType != "PTR") {
		return next(new restify.InvalidArgumentError("You specified an invalid record type ('" + RRType + "'). Currently, only the 'A' (alias) and 'PTR' record types are supported.  e.g. /dns/A/... or /dns/PTR/.."));
	}
	...
	{	
		...
		delexec = 'dnscmd /recorddelete ' + ZoneName + ' ' + NodeName.split(/\.(.+)/)[0] + ' ' + RRType + ' /f';
		addexec = 'dnscmd /recordadd ' + ZoneName + ' ' + NodeName.split(/\.(.+)/)[0] + ' ' + RRType + ' ' + IpAddress;
	}
	else if(RRType == "PTR")
	{
