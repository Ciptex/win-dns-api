...
function doDnsSet(req, res, next) {
	...
	if( RRType != "A" || RRType != "PTR") {
		return next(new restify.InvalidArgumentError("You specified an invalid record type ('" + RRType + "'). Currently, only the 'A' (alias) and 'PTR' record types are supported.  e.g. /dns/my.zone/A/.. or /dns/255.0.10/PTR/.."));
	}
	...
	var ZoneName, delexec, addexec;
	if(RRType == "A")
	{	
		ZoneName = NodeName.split('.',2)[1];
		// Validate Zone Name
		...
		delexec = 'dnscmd /recorddelete ' + ZoneName + ' ' + NodeName.split('.',2)[0] + ' ' + RRType + ' /f';
		addexec = 'dnscmd /recordadd ' + ZoneName + ' ' + NodeName.split('.',2)[0] + ' ' + RRType + ' ' + IpAddress;
	}
	else if(RRType == "PTR")
	{
...
// - For 'set'
server.get('/dns/:type/:node/set/:ip', doDnsSet);
server.head('/dns/:type/:node/set/:ip', doDnsSet);
